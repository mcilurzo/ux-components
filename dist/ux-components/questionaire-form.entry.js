...
const questionaireFormCss = ".question{margin-bottom:24px}.question sbb-title{margin-block-start:0}.summary{display:grid;width:100%;grid-template-columns:1fr 1fr;grid-template-rows:1fr auto;gap:16px 16px;grid-template-areas:\"recommendation history\" \"back back\"}.back{grid-area:back;text-align:right}.recommendation{grid-area:recommendation}.history{grid-area:history}p{margin:0 0 var(--sbb-spacing-fixed-4x) 0}p.answer{margin-bottom:0}.button-left{margin-right:var(--sbb-spacing-fixed-4x)}";
...
const QuestionaireForm = class {
  ...
  render() {
    if (this.currentStep >= this.questions.length) {
      return (h("sbb-group", { color: "milk", padding: "l-l" }, h("div", { class: "summary" }, h("sbb-group", { color: "white", padding: "s-s", class: "history" }, this.answerHistory.map((entry) => (h("div", { class: "question" }, h("sbb-title", { level: "5" }, entry.question), h("p", { class: "answer " + entry.answer }, this.getLocalizedAnswer(entry.answer)))))), h("div", { class: "recommendation" }, h("p", null, this.getRecommendation())), h("div", { class: "back" }, h("sbb-button", { variant: "secondary", size: "m", onClick: () => this.reset() }, this.getLocale().reset)))));
    }
